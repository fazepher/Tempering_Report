...
get_pt_rep <- function(k, pt_fit, C, t, w){
  cycle_length <- t + w
  d <- dim(pt_fit$x)[3]
  x <- matrix(nrow = C*cycle_length, ncol = d)
  for(c in 1:C){
...
replicate_pt_erg <- function(r){
  ...
  pt_rep <- tempeRing:::PT_rwm_chain(
    ...
    beta_schedule = 0.6^(0:4),
    scale = list(5.5, 7.5, 10, 15, 25), 
    Cycles = 1051, Temp_Moves = 1, Within_Moves = 5, 
    silent = TRUE)
  
  pt_rep_erg <- mutate(get_pt_rep(1, pt_fit = pt_rep, C = 1050, t = 1, w = 5), r = r)
...
plan(multisession, workers=15)
pt_erg_reps <- future_map_dfr(1:300, replicate_pt_erg, .options = furrr_options(seed=TRUE))
...
```{r pt-reps-fig}
#| label: fig-pt-reps
#| fig-cap: Ergodic Averages for 300 replications of Parallel Tempering targetting the normal mixture. The gray lines represent the evolution of the averages; the blue diamonds the final cummulative mean; the orange horizontal line is the reference of the true value of the mean. 

pt_erg_reps_plot <- pt_erg_reps |> 
  ggplot(aes(x=s,y=mean,group=r)) + 
  geom_hline(yintercept = 0, color = naranja) + 
  geom_path(color = "gray85", alpha = 0.3, size = rel(0.4)) + 
  geom_point(data = pt_erg_reps_final, size = rel(3), 
             color = azul, alpha = 0.3, shape = 18) + 
  labs(title = "Ergodic Averages of 300 replications", subtitle = "(target level only)") 

pt_erg_reps_plot
```

Just as with ST we report the empirical RMSE, in this case  `r round(sqrt(mean(pt_erg_reps_final$mean^2)),3)`.